  ...
  private void tokenize(Character c, Integer index, Integer lineNumber) {
      if (c.toString().equals("\"")){
          ...
          if (accumIsKeyboard()) {
            ...
            if (state.equals(StateType.NUMBER) && !isNumber(c)) createToken(TokenType.LITERAL, index,lineNumber);
            else if (state.equals(StateType.IS_LETTER)) tokens.add(Token.builder().value(accum).type(TokenType.IDENTIFIER).index(index).line(lineNumber).build());
          tokens.add(Token.builder().value(c.toString()).type(keyWords.get(c.toString())).index(index).line(lineNumber).build());
      }
